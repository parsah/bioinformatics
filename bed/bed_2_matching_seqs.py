...
def run(bed, genome):
    for bed_key in bed:
        ...
        has_match = False # flag to help deduce whether a corresponding sequence has been found.
        suitable_seq = None # represents matching-GC, matching-N, and length control.
        while not has_match:
            ...
            for idx in range(len(cont_chrom_seq)):
                ...
                is_match = is_match_gc(bed_value['gc_perc'], cont_gc) and is_match_n(bed_value['n_perc'], cont_n)
                if is_match:
                    has_match = True
                    suitable_seq = cont_seq # set the control sequence
                    print('>matched.' + bed_key + '\n' + suitable_seq) # print-out
                    break
