'''
A helpful script that performs local BLAST given a valid BLAST local database
and a user-provided query FASTA file.
'''
...
# BLAST hits must not contain these keywords
AMBIGIOUS_KEYWORDS = ['hypothetical', 'putative', 'unknown', 'unnamed',
                     'predicted', 'uncharacterized', 'Predicted']

BLAST_EVALUE = ''  # BLAST e-value cutoff
DB_FOLDER, DB_NAME = '', ''  # BLAST database folder and name, respectively.
INPUT_FILE, NUM_PROCESSES = '', 0  # input FASTA and number of workers sought.
NUM_ENTRIES, CURR_COUNTER = 0, 0  # trivial counters to keep-track of progress
NUM_WITH_HIT, NUM_WITHOUT_HIT = 0, 0  # counters to keep track of hit-count.


def callback(return_from_blast):
    '''
    Multiprocessing callback function that handles output from each worker.
    @param return_from_blast: BioPython BLAST return object.
    @return: string representing the parsed BLAST output.
    '''
    ...
    OUT_HANDLE.flush()
    os.remove(fasta_file)  # at the end, remove the fasta and xml file
    os.remove(xml_file)
    ...
    for hit in all_hits:  # for each hit...
        ...
        for ambig_word in AMBIGIOUS_KEYWORDS:
            if ambig_word in desc:  # if an ambigious word is in the hit,
                is_ambigious_hit = True  # flag,  filter when hits are done
        if not is_ambigious_hit:  # if hit is perfectly valid.
...
def execute_program():
    ...
    for fasta_id in dict_ids_seqs:
        ...
        pool.apply_async(run_blast, args=(fasta_id.replace('>', ''), seq),
                         callback=callback)
    pool.close()
    pool.join()
    pool.terminate()
    ...
    OUT_HANDLE.close()
    print('\n', '=' * 20, BLAST_PROG, 'of', NUM_ENTRIES, 'seqs OK', '=' * 20)
    print('total time: %.4f sec' % (time.time() - start_time),\
        '[ HIT:', NUM_WITH_HIT, '/', NUM_ENTRIES, ';', 'NO HIT:',
        NUM_WITHOUT_HIT, '/', NUM_ENTRIES, ']')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-in', help='input fasta file', dest='in_file')
    parser.add_argument('-prog', help='blast program', dest='prog')
    parser.add_argument('-evalue', help='blast evalue', default='1e-5')
    parser.add_argument('-num_proc', help='#/processes', default=8, type=int)
    parser.add_argument('-blast_db', help='db folder', dest='db_folder')
