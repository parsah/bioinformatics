...
PWM variant is enriched at one instance but another. This 
'''

import pandas
import argparse

id_delim = '..' # tfSearch PWM IDs are delimited by 2x periods.
...
def get_ids(df):
    ...
    '''
    ids = [an_id.rstrip('.').rstrip('..').split(id_delim)[-1] for an_id in df.index]
...
def aggregate(df):
    ...
    for i in ids:
        ...
        for linenum, idx in enumerate(df.index):
            if str(idx).endswith(id_delim + i + '.'):
